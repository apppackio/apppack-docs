...
# Matches a version heading such as "## [1.2.3] - 2024-01-01" or "## 1.2.3".
...
class ChangelogParser:
    ...
    def parse_changelog(
        ...
        parsed_versions = {v for v in changelog_dict if v != "metadata"}
        for version in sorted(self._declared_versions(content) - parsed_versions):
            logger.warning(
                "%s changelog: version %s was not understood and has been left out "
                "(a version heading needs a release date, e.g. '## [%s] - 2024-01-31')",
                repo_name,
                version,
                version,
            )
        ...
        for version, data in changelog_dict.items():
            ...
            try:
                ...
                logger.warning(
                    "%s changelog: version %s has an unusable release date %r "
        ...
        if not entries:
            logger.warning(
                "%s changelog: no versions could be parsed from it at all", repo_name
            )

        logger.info("Parsed %d versions from %s", len(entries), repo_name)
        return entries

    def _declared_versions(self, content: str) -> set[str]:
        """Version headings present in the source, whether or not they parsed."""
        return {
            match.group(1)
            for match in VERSION_HEADING.finditer(content)
...
def prune_stale_version_pages(
    ...
    """Delete version pages that no longer correspond to a parsed version.

    Versions get yanked, aliases get renamed and repositories get dropped from
    the config. Without this, the pages they generated stay published forever.
    """
    expected = {f"{entry.version_id}.md" for entry in entries}
    for page in sorted(versions_dir.glob("*.md")):
        if page.name not in expected:
            logger.info("Removing stale version page %s", page.name)
            page.unlink()
